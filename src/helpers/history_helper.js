...
function addToDict(assetId, amount, dict, utxo, addresses) {
    ...
    if (dict[assetId]) {
        ...
        var addrDiff = addresses.filter(function (addr) { return !dict[assetId].addresses.includes(addr); });
...
function getLossNFT(tx, wallet) {
    ...
    var addrsStripped = walletAddrs.map(function (addr) { return addr.split('-')[1]; });
    ...
    var nfts = inputs.filter(function (input) {
        ...
        if (type === AVMConstants.NFTXFEROUTPUTID)
            return true;
        return false;
    });
    ...
    for (var i = 0; i < nfts.length; i++) {
        ...
        var intersect = owners.filter(function (addr) { return addrsStripped.includes(addr); });
        // Did we lose it?
        if (intersect.length > 0) {
            ...
            for (var n = 0; i < nftsOuts.length; n++) {
                ...
                var addrNotAdded = nftOut.addresses.filter(function (addr) { return !loss.addresses.includes(addr); });
                if (doesMatch) {
    ...
    for (var i = 0; i < nftsOuts.length; i++) {
        ...
        if (intersect.length > 0) {
            ...
            for (var n = 0; n < nftsIns.length; n++) {
                ...
                var addrNotAdded = nftIn.addresses.filter(function (addr) { return !gain.addresses.includes(addr); });
                if (doesMatch) {
...
function getLoss(tx, wallet) {
    ...
    if (ins) {
        var _loop_1 = function (i) {
            ...
            var intersect = addrs.filter(function (addr) { return addrsStripped.includes(addr); });
            if (intersect.length === 0)
                return "continue";
            var assetId = utxo.assetID;
            ...
            outs.forEach(function (utxo) {
                if (utxo.assetID === assetId) {
                    ...
                    var targets = outAddrs.filter(function (addr) { return !addrsStripped.includes(addr) && !receivers.includes(addr); });
        ...
        var _loop_2 = function (i) {
            ...
            ins.forEach(function (input) {
                ...
                if (utxo.assetID === assetId) {
                    ...
                    var targets = outAddrs.filter(function (addr) { return !addrsStripped.includes(addr) && !senders.includes(addr); });
