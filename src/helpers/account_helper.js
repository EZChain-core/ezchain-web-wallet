...
export var checkIfSavedLocally = function (allWallets) {
    ...
    var ethAddressArray = allWallets.map(function (x) {
        return x.ethAddress
    })
    var savedAccounts = getLocalStorageJSONItem('accounts')
    try {
        for (
            var savedAccounts_1 = __values(savedAccounts),
                savedAccounts_1_1 = savedAccounts_1.next();
            !savedAccounts_1_1.done;
            savedAccounts_1_1 = savedAccounts_1.next()
        ) {
        ...
        try {
            if (savedAccounts_1_1 && !savedAccounts_1_1.done && (_a = savedAccounts_1.return))
                _a.call(savedAccounts_1)
        } finally {
...
export var getIndexByWallets = function (wallets) {
    var ethAddressArray = wallets.map(function (x) {
        return x.getEvmAddress()
    })
