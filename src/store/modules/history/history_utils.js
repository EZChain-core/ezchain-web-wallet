...
export function isArraysOverlap(arr1, arr2) {
    var overlaps = arr1.filter(function (item) { return arr2.includes(item); });
    return overlaps.length > 0;
}
// To get the stake amount, sum the non-reward output utxos.
export function getStakeAmount(tx) {
    var nonRewardUtxos = tx.outputs.filter(function (utxo) { return !utxo.rewardUtxo && utxo.stake; });
...
export function getAddresses(outs) {
    ...
    // Remove duplicated
    return allAddrs.filter(function (addr, i) { return allAddrs.indexOf(addr) === i; });
}
/**
...
export function getAssetOutputs(outs, assetID) {
    return outs.filter(function (out) { return out.assetID === assetID; });
}
...
export function getRewardOuts(outs) {
    return outs.filter(function (out) { return out.rewardUtxo; });
}
...
export function avaxTransferDataToCsvRow(rowData) {
    ...
    var froms = rowData.from ? "\"" + ((_a = rowData.from) === null || _a === void 0 ? void 0 : _a.join('\n')) + "\"" : '-';
    var tos = rowData.to ? "\"" + ((_b = rowData.to) === null || _b === void 0 ? void 0 : _b.join('\n')) + "\"" : '-';
...
export function parseMemo(memoRaw) {
    ...
    if (!memoText.length || memoRaw === 'AAAAAA==')
        return '';
    return memoText;
}
