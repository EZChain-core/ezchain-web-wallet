...
var LedgerWallet = /** @class */ (function (_super) {
    ...
    LedgerWallet.prototype.getTransactionPaths = function (unsignedTx, chainId) {
        ...
        for (let i = 0; i < operations.length; i++) {
            ...
            let sources = sigidxs.map(function (sigidx) {
                return sigidx.getSource()
            })
            let addrs = sources.map(function (source) {
                return bintools.addressToString(hrp, chainId, source)
            })
    ...
    LedgerWallet.prototype.getCredentials = function (unsignedTx, paths, sigMap, chainId) {
        ...
        for (let i = 0; i < operations.length; i++) {
            ...
            let sigidxs = op.getSigIdxs()
            let cred = CredentialClass(op.getCredentialID())
        ...
        for (let i = 0; i < evmInputs.length; i++) {
            ...
            let sigidxs = evmInput.getSigIdxs()
            let cred = CredentialClass(evmInput.getCredentialID())
            for (let j = 0; j < sigidxs.length; j++) {
                let pathIndex = items.length + i + j
                let pathStr = paths[pathIndex]
                let sigRaw = sigMap.get(pathStr)
    ...
    LedgerWallet.prototype.getValidateDelegateMsgs = function (unsignedTx, chainId) {
        ...
        if (
            ...
            messages.push({ title: 'Total Stake', value: stakeAmt + ' EZC' })
            messages.push({
                title: 'Stake',
    ...
    LedgerWallet.prototype.getFeeMsgs = function (unsignedTx, chainId) {
        ...
        if (
            ...
            messages.push({ title: 'Fee', value: 0.001 + ' EZC' })
        }
        return messages
    }
